...
from model.IntTowerOld import IntTowerOld
...
if __name__ == "__main__":
    ...
    mdckpt2 = ModelCheckpoint(filepath='movie_IntowerOld.ckpt', monitor='val_ndcg',
                        mode='max', verbose=1, save_best_only=True, save_weights_only=True)
    mdckpt3 = ModelCheckpoint(filepath='movie_deepfm.ckpt', monitor='val_ndcg',
                        mode='max', verbose=1, save_best_only=True, save_weights_only=True)
    mdckpt4 = ModelCheckpoint(filepath='movie_dssm.ckpt', monitor='val_ndcg',
                        mode='max', verbose=1, save_best_only=True, save_weights_only=True)
    ...
    model.compile("adam", "binary_crossentropy", metrics=['recall','ndcg']
                , lr=lr)
    ...
    model2 = IntTowerOld(user_feature_columns, item_feature_columns, field_dim= 32, task='binary', dnn_dropout=dropout,
                device=device, user_head=6,item_head=6,user_filed_size=4,item_filed_size=2)
    model2.compile("adam", "binary_crossentropy", metrics=['recall','ndcg']
            , lr=lr)
    model2.fit(train_model_input, train[target].values, batch_size= batch_size, epochs=epoch, verbose=2, validation_split=0.2,
            callbacks=[es, mdckpt2])
    model2.load_state_dict(torch.load('movie_IntowerOld.ckpt'))
    model2.eval()


    model3 = DeepFM(linear_feature_columns, dnn_feature_columns, task='binary', dnn_dropout=0.1,
                    device=device)
    model3.compile("adam", "binary_crossentropy", metrics=['recall','ndcg']
                , lr=lr)
    model3.fit(train_model_input, train[target].values, batch_size= batch_size, epochs=epoch, verbose=2, validation_split=0.2,
              callbacks=[es2, mdckpt3])
    model3.load_state_dict(torch.load('movie_deepfm.ckpt'))
    model3.eval()


    model4 = DSSM(user_feature_columns, item_feature_columns, task='binary', device=device)
