...
from preprocessing.utils import cossim_score
...
class IntTower(BaseTower):
    ...
    def forward(self, inputs):
        ...
        if len(self.user_dnn_feature_columns) > 0:
            ...
            self.user_fe_rep = self.user_fe_dnn(user_dnn_input) # [3, B, #head * field_dim]
            user_embeddings = []
            for i in range(3):
                user_temp = torch.reshape(self.user_fe_rep[i], (-1, self.user_head, self.field_dim))
                user_embeddings.append(user_temp)
            self.user_dnn_embedding = torch.stack(user_embeddings).sum(0).sum(1) #[B, field_dim]
        ...
        if len(self.item_dnn_feature_columns) > 0:
            ...
            self.item_fe_rep = self.item_fe_dnn(item_dnn_input) # [1, B, #head*field_dim]
            self.item_dnn_embedding = torch.reshape(self.item_fe_rep[-1], (-1, self.item_head, self.field_dim)).sum(1)


        if len(self.user_dnn_feature_columns) > 0 and len(self.item_dnn_feature_columns) > 0:
            score = torch.mul(self.user_dnn_embedding, self.item_dnn_embedding).sum(1)
