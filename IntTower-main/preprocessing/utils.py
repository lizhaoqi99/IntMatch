...
def cossim_score(user_embedding, item_embedding):
    # [B, field_dim]
...
def fe_score(user_rep, item_rep, user_fea_col, item_fea_col, user_embedding_dim, item_embedding_dim):
    ...
    item_embedding = torch.reshape(item_rep[-1], (-1, item_fea_col, item_embedding_dim[i])).sum(1)
    user_embedding = torch.stack(user_embeddings).sum(0).sum(1)

    # score=(user_embedding @ item_embedding.permute(0, 2, 1)).values.sum(1)
    score = torch.mul(user_embedding, item_embedding)
